...
void show_flights_shortest_distance_1connection(char *origin, char *destiny, char *sort_type, Flight_list *flights_list ) {
    ...
    while (sorted_flight != NULL) {
        ...
        while(sorted_flight2 != NULL) {
            ...
            (sorted_flight->arrival_time_hour == sorted_flight2->depart_time_hour &&
            sorted_flight->arrival_time_minute < sorted_flight2->depart_time_minute))) {
                if (distance_min == 0 || sorted_flight->distance + sorted_flight2->distance < distance_min) {
                   distance_min = sorted_flight->distance + sorted_flight2->distance;
               }
            }
            sorted_flight2 = sorted_flight2->next;
        }
    ...
    Flight *sorted_flight3 = flights_list->head;
    while (sorted_flight3 != NULL) {
        Flight *sorted_flight4 = flights_list->head;
            while(sorted_flight4 != NULL) {
                if (!strcmp(sorted_flight4->depart_IATA, origin) && 
                !strcmp(sorted_flight3->depart_IATA, sorted_flight4->arrival_IATA) &&
                (sorted_flight4->arrival_time_hour < sorted_flight3->depart_time_hour ||
                (sorted_flight4->arrival_time_hour == sorted_flight3->depart_time_hour &&
                sorted_flight4->arrival_time_minute < sorted_flight3->depart_time_minute))) {
                    if (sorted_flight4->distance + sorted_flight3->distance == distance_min) {
                        print_flight_information(sorted_flight4);
                        printf("connects to ");
                        print_flight_information(sorted_flight3);
                        printf("\n");
                    }
                }
                sorted_flight4 = sorted_flight4->next;
            }
        sorted_flight3 = sorted_flight3->next;
    }
    free_flights_list(sorted_flights);
